...
<body ontouchstart="return true;">
	...
	<header>
		<div class="header">
			<h1>编辑收货地址</h1>
			<a href="{{ url('home/user/dizhi') }}" class="return"><i></i></a>
			<a onclick="demo();" class="head-r">保存并使用</a>
		</div>
	</header>
	...
	<article class="main-container">
		...
		<div class="list list-title">
			<ul class="line">
				<li><i>收&nbsp;&nbsp;货&nbsp;&nbsp;人：</i><input type="text" id="name" name="name" value="{{ $list->name }}" placeholder="名字"></li>
				<li><i>联系电话：</i><input type="tel" id="name" name="phone" value="{{ $list->phone }}" placeholder="手机或固话"></li>
				<li class="list03"><i>所在地区：</i>
					<select name="select" class="cid" id="city">
						<option  selected="selected">{{ $list->city }}</option>
					</select>
				</li>
				<li><i>详情地址：</i><input type="text" id="name" name="detail" value="{{ $list->detail }}" placeholder="街道门牌信息"></li>
			</ul>
		</div>
...
<script type="text/javascript" >
    ...
    $("select").live("change",function(){
        ...
        var ob = $(this);
        //判断选择的是不是默认的请选择
        if($(this).val() != '请选择' && $(this).val() != "{{ $list->city }}"){
            var upid = $(this).val();
            $.ajax({
    ...
    function demo()
	{
		...
		if(name == null || phone == null || detail == null || city == null || xian == null){
			alert("请将信息填写完整!");
		}else{
		    var id = {{ $list->id }};
			$.ajax({
                url:'{{ url("home/user/ajax/changedizhi") }}',
                async:false,				//是否异步
                type:'post',				//请求方式
                data:{id:id, name:name, phone:phone, detail:detail, dizhi:dizhi, '_token': $('input[name=_token]').val()},				//发送的数据
                dataType:'json',			//响应的数据类型
                success:function(data)		//成功时回调函数
                {

                    if(data == '地址编辑成功!'){
                        location.href = "{{url('home/user/dizhi') }}";
                    }else{
                        alert(data);
					}
                }
