...
<script type="text/javascript" src="{{ asset('myjquery-1.8.3.min.js') }}"></script>
<script type="text/javascript">
    function demo(bn){
        var name = $(bn).attr('name');
        var val = $(bn).val();
        $.ajax({
            url:"{{url('home/user/ajax/resgName') }}",
            async:false,
            data:{'name':name, 'value':val, '_token':$('input[name=_token]').val()},
            type:"post",
            dataType:"json",
            error:function(){
...
<header>
  <div class="header">
    <h1>注册</h1>
    <a href="#" class="return"><i></i></a> <a href="{{ url('home/user/login') }}" class="head-r">登录</a> </div>
</header>

<!--内容区-->

<article class="main-container loginContent" style="margin-top:50px;">
    <input type='hidden' name='_token' value='{{ csrf_token() }}'>
  <div class="list">
    <ul class="line">
      <li><i class="icon-head"></i>
        <input type="text" id="name" name="username" placeholder="手机号码" onblur="demo(this);">
      </li>
      <li class="code"><i class="icon-sms"></i>
        <input type="tel" id="code" placeholder="验证码">
        <button>点击获取</button>
      </li>
      <li><i class="icon-pwd"></i>
        <input type="password" id="zxc" name="password" placeholder="密码" >
      </li>
       <li class="code"><i class="icon-pwd"></i>
        <input type="password" id="code" name="pwd" placeholder="确认密码" onblur="pass(this);">
      </li>
      {{--<li class="code"><i class="icon-invite"></i>--}}
        {{--<input type="tel" id="code" placeholder="邀请码">--}}
      {{--</li>--}}
    </ul>
  </div>
  <div class="button">
    <button onclick="regis(this)">立即注册</button>
  </div>
</article>
