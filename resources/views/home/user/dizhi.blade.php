...
<link rel="stylesheet" href="{{ asset('myhome/css/responsive.css') }}">
	<script>
		function change(id,bn)
		{
		    if($(bn).attr('class') != 'icon-correct') {
                if (confirm("你确定要将该地址变为收货地址吗？")) {
                    $.ajax({
                        url: '{{ url("home/user/ajax/defdizhi") }}',
                        async: false,				//是否异步
                        type: 'post',				//请求方式
                        ...
                        {
                            if (data == true) {
                                $('.icon-correct').css({background: 'rgba(240, 240, 240, 1)'});
                                $('.icon-correct').attr({class:'icon-error'});
                                $(bn).css({background: 'rgba(255, 159, 0, 1)'});
                                $(bn).attr({class:'icon-correct'});
                            } else {
                                alert("该功能正在维护中!");
                            }
                        }
        ...
        function doDel(id){
            if(confirm("你确定要删除该地址吗？")){
                location.href = '/home/user/deldizhi/'+id;
            }
        }
	</script>
</head>
...
<body ontouchstart="return true;">
	...
	<header>
		<div class="header">
			<h1>收货地址</h1>
			<a href="{{ url('home/user') }}" class="return"><i></i></a>
			<a href="{{ url('home/user/adddizhi') }}" class="head-r">添加</a>
		</div>
	</header>
	...
	<article class="main-container">
		...
		@if($list != null)
			@foreach ($list as $v)
				...
				@if($v->status == 2)
					<dd onclick="change({{ $v->id }},this);" class="icon-correct"></dd>
				@else
					<dd onclick="change({{ $v->id }},this);" style="background: rgba(240, 240, 240, 1);" class="icon-error"></dd>
				@endif
					</dl>
					<ul class="line"><li class="line"><a href="{{ url('home/user/updadizhi/'.$v->id) }}"><button>编辑</button></a></li><li><button onclick="doDel({{ $v->id }})">删除</button></li></ul>
				</div>
			@endforeach
		@else
			<div class="order-Address line select"><dl><div align="center">还没有地址，请添加地址</div></dl></div>
		@endif
	</article>
