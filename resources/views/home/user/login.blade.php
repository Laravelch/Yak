...
<!-- 页面描述 -->
<meta name="description" content="我们是药快"/>
<!-- 页面关键词 -->
      ...
      function login(){
             ...
             if(username == '' || pwd == ''){
                alert("请将信息填写完整在登录！");
                 ...
                 $.ajax({
                     url: "{{url('home/user/ajax/login')}}",
                     async: true,
                     data: {'username': username, 'password': pwd, '_token': $('input[name=_token]').val()},
                     type: "post",
                     dataType: "json",
                     error: function () {
...
<header>
  <div class="header">
    <h1>登录</h1>
    <a href="#" class="return"><i></i></a> <a href="{{url('home/user/register')}}" class="head-r">注册</a> </div>
</header>
  ...
  <div class="list">
    <ul class="line">
      <li><i class="icon-head"></i>
        <input type="text" id="name" name="username" placeholder="手机号码" >
      </li>
      <li class="pwd"><i class="icon-pwd"></i>
        <input type="password" name="password" placeholder="密码" >
        {{--<button onClick="window.location='repass.html'">忘记密码？</button>--}}
      </li>
      <!-- <li class="code"><i class="icon-sms"></i>
        <input type="text" id="code" placeholder="校验码">
        <button>点击获取</button>
      </li>-->
    </ul>
  </div>
