...
<link rel="stylesheet" href="{{ asset('myhome/css/responsive.css') }}">
	...
	<script type="text/javascript">
        function pass(bn){
            var pwd = $(bn).val();
            var password = $("input[name=xpassword]").val();
        ...
        function regis() {
            ...
            if (password == '' || xpassword == '' || qpassword == '') {
                alert("请将信息填写完整！");
            }else{
                $.ajax({
                    url: "{{url('home/user/ajax/pwd') }}",
                    async: true,
                    data: {'password': password, 'xpassword': xpassword, '_token': $('input[name=_token]').val()},
                    type: "post",
                    dataType: "json",
...
<body ontouchstart="return true;">
	...
	<header>
		<div class="header">
			...
			<a onclick="regis();" class="head-r">保存</a>
		</div>
	</header>
	...
	<article class="main-container">
		...
		<div class="list">
			<ul class="line">
				<li><i class="icon-pwd"></i><input type="password" id="name" name="password" placeholder="原始密码" ></li>
				<li><i class="icon-pwd"></i><input type="password" id="name" name="xpassword" placeholder="填写新密码" ></li>
				<li><i class="icon-pwd"></i><input type="password" id="name" name="qpassword" placeholder="确认新密码" onblur="pass(this);"></li>
			</ul>
		</div>
